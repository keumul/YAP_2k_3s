...
namespace IT
{
	...
	void showTable(IdTable& idtable)
	{
		...
		for (i = 0; i < idtable.size; i++)
		{
			...
			default: std::cout << std::setw(10) << std::left << "unknown" << " | "; break; //если варианты не подходят
			}
			...
			case OP: std::cout << std::setw(18) << std::left << "оператор" << " | ";
				number++;
				break;
			default: std::cout << std::setw(18) << std::left << "unknown" << " | "; break; //если варианты не подходят
			}
